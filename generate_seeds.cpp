...
//   // std::default_random_engine rng(rd());
  // Use fixed output for fair testing
  std::default_random_engine rng(0);
  std::uniform_int_distribution<> dist(0,sizeof(alphabet)/sizeof(*alphabet)-2);

  std::vector<std::string> x_roots;
  x_roots.reserve(N_STRS);
  std::generate_n(std::back_inserter(x_roots), x_roots.capacity(),
      [&] { std::string str; 
            str.reserve(S_LEN); 
            ...
            return str; });
  int i = 0;
  for (vector<string>::iterator x=x_roots.begin(); x!=x_roots.end(); ++x, ++i) 
  {
      std::cout << *x << std::endl;
      // std::cout << i << ",";
      // auto challenge_hash_bits = std::vector<uint8_t>(x->begin(), x->end());
